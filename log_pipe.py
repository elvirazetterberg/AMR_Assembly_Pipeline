...
def log_entry(string,log):
    time = currenttime()
    log.writelines(f'{time}: {string}\n')    
    return

def log_parse(string):
    time = currenttime()
    os.system(f"echo {time}: {string} >> {LOGFILE}")
    return

def test_log(log):
    log_entry(f'TESTING!\n'*10, log)
    output = sp.getoutput('conda info --envs')
    print (output)
    log_entry(f'{output}!\n'*10, log)
    return


def ariba_fun(infile1,infile2,db_ariba,log):
    for db_name in db_ariba[1:-1].split(','): #klumpigt? as sysargv makes input a string, it is separated into a list here. Also parallell should do all db at same time?
        ...
        print (output)
        log_entry(f'{currenttime()}: Starting ariba with {db_name}', log)
        if os.path.exists(f'out.{db_name}.fa'): 
            log_entry(f'{currenttime()}: Database {db_name} allready downloaded\n', log)
            ...
            log_entry(f'{currenttime()} Downloading database {db_name}\n', log)
...
# Ariba 
    if ariba:
        log.writelines(f'=================================\n{currenttime()}: Ariba\n=================================\n') # omformatera till ett sammanhängande format
        ariba_fun(infile1,infile2, db_ariba,log)
